import { validateSubCategory } from '@/lib/validateCategory'
...
type CategoryFormData = z.infer<typeof validateSubCategory>
...
const fieldConfigs = [
  {
    name: 'subCategory_name' as keyof CategoryFormData,
    label: 'subCategory Name',
    placeholder: 'Enter subCategory Name',
    validator: (value: string) => {
      const result = validateSubCategory.shape.subcategory_name.safeParse(value)
  ...
  },
  {
    ...
    validator: (value: string) => {
      const result = validateSubCategory.shape.description.safeParse(value)
      return result.success ? undefined : result.error.issues[0]?.message
    },
  },
]
...
function SubCategoryModal(
    ...
    const { Field, reset, Subscribe, handleSubmit } = useForm({
        ...
        onSubmit: async ({ value }) => {
            const payload = { ...value, category_id: id }
            const validate = validateSubCategory.safeParse(payload)
            if (!validate.success) {
                            ...
                            <Field
                                ...
                                validators={{
                                    onChange: ({ value }) =>
                                        validateField(
                                            value,
                                            validateSubCategory.shape[config.name],
                                        ),
                                }}
                                children={(field) => (
                                    <div className="mb-4">
